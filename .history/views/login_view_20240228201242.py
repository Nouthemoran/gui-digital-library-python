...
def login_page():
    ...
    username_label.grid(row=0, column=0, padx=100, pady=3)
    username_entry = Entry(login_window)
    username_entry.grid(row=0, column=1, padx=100, pady=3)

    # Label dan input field untuk password
    password_label = Label(login_window, text="Password:")
    password_label.grid(row=1, column=0, padx=100, pady=3)
    password_entry = Entry(login_window, show="*")
    password_entry.grid(row=1, column=1, padx=100, pady=3)

    # Tombol untuk melakukan login
    login_button = Button(login_window, text="Login", command=validate_login)
    login_button.grid(row=2, column=0, columnspan=2, padx=100, pady=3, sticky="WE")
